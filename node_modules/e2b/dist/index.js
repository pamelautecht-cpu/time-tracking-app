...
var Commands = class {
  ...
  async start(cmd, opts) {
    ...
    if ((opts == null ? void 0 : opts.stdin) === false && (0, import_compare_versions2.compareVersions)(this.envdVersion, ENVD_COMMANDS_STDIN) < 0) {
...
var SandboxApi = class {
  ...
  static async createSandbox(template, timeoutMs, opts) {
    ...
    if ((0, import_compare_versions3.compareVersions)(res.data.envdVersion, "0.1.0") < 0) {
...
var Sandbox = class extends SandboxApi {
  ...
  async getMetrics(opts) {
    ...
    if (this.envdApi.version) {
      if ((0, import_compare_versions4.compareVersions)(this.envdApi.version, "0.1.5") < 0) {
        throw new SandboxError(
          "You need to update the template to use the new SDK. You can do this by running `e2b template build` in the directory with the template."
        );
      }
      if ((0, import_compare_versions4.compareVersions)(this.envdApi.version, "0.2.4") < 0) {
...
Sandbox.defaultMcpTemplate = "mcp-gateway-v0-1";
...
async function calculateFilesHash(src, dest, contextPath, ignorePatterns, resolveSymlinks, stackTrace) {
  ...
  for (const file of files) {
    const relativePath = import_node_path.default.relative(contextPath, file.fullpath());
...
async function tarFileStream(fileName, fileContextPath, resolveSymlinks) {
...
async function tarFileStreamUpload(fileName, fileContextPath, resolveSymlinks) {
...
var TemplateBase = class {
  ...
  static async build(template, options) {
    ...
    try {
      ...
      return await template.build(options);
    } finally {
      (_b = options.onBuildLogs) == null ? void 0 : _b.call(options, new LogEntryEnd(/* @__PURE__ */ new Date(), "Build finished"));
    }
  }
